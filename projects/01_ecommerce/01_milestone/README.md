...
### **Core Concepts**
#### **1.Naming Convention:** File name should ve First letter capital and then with uppercase first letter with every changed work, usually say PascalCase. Same when naming a Component.
...
#### Let's explain above code: ####
  ...
  - **Line 2:** Return the html element h1, interestingly react treat html tags as
            valid syntax in Javascript.
  - **Line 4:** With "export" keyword exposed this component outside world 
              so it can be use with import statement in other pages/components. 
              Keyword default means here that this would be a default import component. 
...
- **Example**:
  ...
  function WelcomeMessage({ firstName, lastName  }) { //L1
    return <h2>Welcome, {firstName} {lastName}!</h2>; //L2
  } //L3
...
- **L1:** declare component name, pass an object as function parameter. 
...
- **L2:** Interesting :) h2 html element with dynamic values or can say
        variables. So here to print/show the value of the variable we use curly
        braces {}. i.e. `{firsname}` `{lastName}`

        
- **L4:** export default to expose it to import in other componenets.  
...
- **L1:** import Welcome Message component to use it.
- **L2:** Create component
- **L3:** Interesting :) Use WelcomeMessage component to just like html tag and pass two attributes/properties with their respective values. `firstName`, `lastName` are attributes/properties here. 
...
- **Code Explanation:** 
- **L3 & L4:** Line 3 and Line 4 are just simple constant values declaration and assigment.  
- **L5:** Interesting :) Props firstName and lastName are passed dynamically to the WelcomeMessage component using curly braces {}.
  Curly braces allow you to pass JavaScript expressions (variables, constants, functions) into JSX. `{firstName}` `lastName` 
...
```jsx
 // ❌ Wrong Way: Do not return multiple elements side-by-side without a wrapper.
    function Header() {
        return <h1>Welcome, </h1><span> John Doe</span>; 
    }
...
- Use Fragments to Avoid Extra DOM Elements: (React Fragements) `<>` `</>`
```jsx
function Header() {
    return (
        <>  //🔔Pay attention on me
...
- Component name should be always start with Capital letter (Pascal Case)
```jsx
 // ❌ Wrong Way:
...
```jsx
function Header() { ... }
export Header; // no default export
...
- Example `width: md:wd-48;` 48 width on medium screen. so 48 * 0.25 = 12rem * 16px = 192px. `rem` here is root element font size which is html here. So html font-size is 16px and 1 rem = 16px  
...
- NodeJs: Its Javascript Runtime environment that allows javascript to run outside of browser. Built on Chrome's Javascript Engine. 
  Installation: https://nodejs.org/en/download
    
- NVM: Node version manager to allow use to work with different node versions.
- Some commands of NVM
```bash
nvm use 18
nvm ls
nvm install 18
...
- `package.json` Track dependencies of the project and scripts to automate tasks like start, build. Helps to port/transfer project as no need to send the required modules as can easily install all dependencies with `npm install`, as a result reduce project size while transfer.
- Versioning in `package.json` i.e. `react ^19.1.0`. It shows `Major.Minor.Patch`.    
...
- `package-lock.json`file, maintain/lock the exact package versions when you run the `npm install`. it regenerates every time with `npm install`but keep the exist version number when we installed any package first time. So package-lock.json file makes sure that we and our colleagues always install the same package versions which are in package-lock.json file. For example if we installed react first time and version is `react ^19.0.1` so it will always be the same.
  > But if we delete the `package-lock.json` file and run npm install then maybe now react version install would be ^19.5.2 the lastest it found for 19 Major version. 
  
- **So deleting package-lock.json could lead to inconsistency and can break the project**


- **Sometimes to solve errors we may need to delete modules folder in our project and may need to run ``npm instal` to install all packages**
...
2. **Set Up a React Project**:
   - Vite is tool to quickly create a project with all required dependencies for all popular frameworks. For example react is a framework.
